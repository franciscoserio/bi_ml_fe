...
class Datasource extends Component {
      ...
      for (var i = 0; i < this.state.datasources.length; i++) {
        if (this.state.datasources[i]["id"] === e.target.value) {
          ...
          for (var j = 0; j < this.state.databases.length; j++) {
            if (
              this.state.datasources[i]["sgbd"] ===
              this.state.databases[j]["name"]
            ) {
  ...
  handleOnClickSaveNewConnection = (e) => {
    ...
    if (name === "") {
      $("#nameNewConnection").css("border-color", "red");
    } else {
      $("#nameNewConnection").css("border-color", "rgb(207, 207, 207)");
      $(".errorAlertNewConnection").css("display", "block");
    }

    if (sgbd === "" || sgbd === "Database") {
      $("#SgbdNewConnection").css("border-color", "red");
      $(".errorAlertNewConnection").css("display", "block");
    } else {
      $("#SgbdNewConnection").css("border-color", "rgb(207, 207, 207)");
    }

    if (host === "") {
      $("#hostNewConnection").css("border-color", "red");
      $(".errorAlertNewConnection").css("display", "block");
    } else {
      $("#hostNewConnection").css("border-color", "rgb(207, 207, 207)");
    }

    if (port === "") {
      $("#portNewConnection").css("border-color", "red");
      $(".errorAlertNewConnection").css("display", "block");
    } else {
      $("#portNewConnection").css("border-color", "rgb(207, 207, 207)");
    }

    if (database === "") {
      $("#databaseNewConnection").css("border-color", "red");
      $(".errorAlertNewConnection").css("display", "block");
    } else {
      $("#databaseNewConnection").css("border-color", "rgb(207, 207, 207)");
    }

    if (user === "") {
      $("#userNewConnection").css("border-color", "red");
      $(".errorAlertNewConnection").css("display", "block");
    } else {
      $("#userNewConnection").css("border-color", "rgb(207, 207, 207)");
    }

    if (password === "") {
      $("#passwordNewConnection").css("border-color", "red");
      $(".errorAlertNewConnection").css("display", "block");
    } else {
      $("#passwordNewConnection").css("border-color", "rgb(207, 207, 207)");
    }
          ...
          <div className="col">
            ...
            <div class="input-group mb-3">
              ...
              <select
                ...
                id="selectSavedConnections"
                onChange={this.handleOnChangeSavedConnection}
              >
                <option defaultValue>Select</option>
                {this.state.datasources.map((datasource, index) => (
                  <option key={index} value={datasource["id"]}>
                    {datasource["name"]}
                  </option>
                ))}
              </select>
            ...
            <div class="input-group mb-3">
              ...
              <select className="custom-select" id="sgbdSavedConnections">
                ...
                {this.state.databases.map((database, index) => (
                  <option key={index} value={database["id"]}>
                    {database["name"]}
                  </option>
                ))}
              </select>
